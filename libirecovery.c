...
int
irecv_control_transfer(irecv_client_t client,
		       uint8_t bmRequestType,
		       uint8_t bRequest,
		       uint16_t wValue,
		       uint16_t wIndex,
		       ...
		       uint16_t wLength, unsigned int timeout)
{
	...
	if (timeout <= 10) {
		...
		struct darwin_device_priv *dpriv =
		    (struct darwin_device_priv *)client->handle->dev->os_priv;
		...
		kresult =
		    (*(dpriv->device))->DeviceRequestAsync(dpriv->device, &req,
							   (IOAsyncCallback1)
							   dummy_callback,
							   NULL);
		usleep(5 * 1000);
		kresult =
		    (*(dpriv->device))->USBDeviceAbortPipeZero(dpriv->device);
		return kresult == KERN_SUCCESS ? 0 : -1;
	} else {
		return libusb_control_transfer(client->handle, bmRequestType,
...
int
irecv_bulk_transfer(irecv_client_t client,
		    unsigned char endpoint,
		    unsigned char *data,
		    int length, int *transferred, unsigned int timeout)
{
#ifndef _WIN32
	return libusb_bulk_transfer(client->handle, endpoint, data, length,
				    transferred, timeout);
#else
	int ret;
	if (endpoint == 0x4) {
...
int
irecv_get_string_descriptor_ascii(irecv_client_t client, uint8_t desc_index,
...
irecv_error_t
irecv_set_interface(irecv_client_t client, int interface, int alt_interface)
{
	if (check_context(client) != IRECV_E_SUCCESS)
		return IRECV_E_NO_DEVICE;
...
irecv_error_t
irecv_event_subscribe(irecv_client_t client, irecv_event_type type,
...
irecv_error_t
irecv_event_unsubscribe(irecv_client_t client, irecv_event_type type)
{
	switch (type) {
	case IRECV_RECEIVED:
...
irecv_send_command_raw(irecv_client_t client, const char *command)
{
...
irecv_error_t
irecv_send_file(irecv_client_t client, const char *filename,
...
irecv_error_t
irecv_send_buffer(irecv_client_t client, unsigned char *buffer,
...
irecv_error_t
irecv_setenv(irecv_client_t client, const char *variable, const char *value)
{
	char command[256];
	if (check_context(client) != IRECV_E_SUCCESS)
		return IRECV_E_NO_DEVICE;
...
irecv_error_t
irecv_recv_buffer(irecv_client_t client, char *buffer, unsigned long length)
{
	//irecv_error_t error = IRECV_E_SUCCESS;
	int recovery_mode = (client->mode != kDfuMode);
