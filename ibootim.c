	...
	if (info->dirty) {
		...
		info->compLength =
		    (uint32_t) (compress_lzss
				(compressed, info->length * 2, info->buffer,
				 info->length) - compressed);
...
AbstractFile *createAbstractFileFromIBootIM(AbstractFile * file)
{
	...
	if (strcmp(info->header.signature, IBOOTIM_SIGNATURE) != 0) {
		free(info);
		XLOG(1,
		     "createAbstractFileFromIBootIM: signature does not match\n");
		return NULL;
	}
	...
	if (info->header.compression_type != IBOOTIM_LZSS_TYPE) {
		//free(info);
		XLOG(1,
		     "createAbstractFileFromIBootIM: (warning) unsupported compression type: %x\n",
	...
	int length =
	    decompress_lzss(info->buffer, compressed, info->compLength);
	if (length > info->length) {
		...
		XLOG(1,
		     "createAbstractFileFromIBootIM: (warning) uncompressed data shorter than expected: %d\n",
		     length);
		info->length = length;
	}
...
int
convertToPNG(AbstractFile * imageWrapper, const unsigned int *key,
	     const unsigned int *iv, const char *png)
{
	AbstractFile *imageFile;
	...
	png_structp png_ptr =
	    png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, pngError,
				    pngWarn);
	if (!png_ptr) {
		return -1;
	}
	...
	png_bytepp row_pointers =
	    (png_bytepp) malloc(sizeof(png_bytep) * info->header.height);
	int i;
	for (i = 0; i < info_ptr->height; i++) {
		row_pointers[i] =
		    imageBuffer + (info->header.width * bytes_per_pixel * i);
	}
...
void *replaceBootImage(AbstractFile * imageWrapper, const unsigned int *key,
		       const unsigned int *iv, AbstractFile * png,
		       size_t * fileSize)
{
	AbstractFile *imageFile;
	...
	png_structp png_ptr =
	    png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, pngError,
				   pngWarn);
	if (!png_ptr) {
		return NULL;
	}
	...
	if (info_ptr->bit_depth > 8) {
		XLOG(0,
		     "warning: bit depth per channel is greater than 8 (%d). Attempting to strip, but image quality will be degraded.\n",
		     info_ptr->bit_depth);
	}
	...
	if (info_ptr->color_type == PNG_COLOR_TYPE_GRAY
		...
		XLOG(0,
		     "notice: attempting to add dummy transparency channel\n");
	}
	...
	if (info_ptr->width > 320 || info_ptr->height > 480) {
		XLOG(0,
		     "error: dimensions out of range, must be within 320x480, not %lux%lu\n",
	...
	if (info_ptr->color_type != PNG_COLOR_TYPE_GRAY_ALPHA
		...
		XLOG(0,
		     "error: incorrect color type, must be greyscale with alpha, or rgb with alpha\n");
		if (info_ptr->color_type == PNG_COLOR_TYPE_GRAY
		    || info_ptr->color_type == PNG_COLOR_TYPE_RGB) {
			XLOG(0,
			     "It appears you're missing an alpha channel. Add transparency to your image\n");
		}
		if (info_ptr->color_type == PNG_COLOR_TYPE_PALETTE) {
			XLOG(0,
			     "This PNG is saved with the palette color type rather than ARGB.\n");
		}
		png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
	...
	row_pointers =
	    (png_bytepp) malloc(sizeof(png_bytep) * info_ptr->height);
	...
	*fileSize = 0;
	...
	if (key != NULL) {
		imageFile =
		    duplicateAbstractFile2(imageWrapper,
					   createAbstractFileFromMemoryFile((void **)
									    &buffer, fileSize), key, iv, NULL);
	} else {
		imageFile =
		    duplicateAbstractFile(imageWrapper,
					  createAbstractFileFromMemoryFile((void
									    **)
									   &buffer, fileSize));
	}
	...
	imageFile->write(imageFile, imageBuffer,
