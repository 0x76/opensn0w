...
void
flipCompHeader (CompHeader * header)
{
...
size_t
readComp (AbstractFile * file, void *data, size_t len)
{
	InfoComp *info = (InfoComp *) (file->data);
	memcpy (data, (void *) ((uint8_t *) info->buffer + (uint32_t) info->offset),
		len);
...
writeComp (AbstractFile * file, const void *data, size_t len)
{
	...
	memcpy ((void *) ((uint8_t *) info->buffer + (uint32_t) info->offset), data,
		len);
	info->offset += (size_t) len;
...
int
seekComp (AbstractFile * file, off_t offset)
{
	InfoComp *info = (InfoComp *) (file->data);
...
off_t
getLengthComp (AbstractFile * file)
{
	InfoComp *info = (InfoComp *) (file->data);
...
closeComp (AbstractFile * file)
{
	...
	(uint32_t) (compress_lzss
				(compressed, info->header.length_uncompressed * 2,
				 info->buffer,
				 info->header.length_uncompressed) - compressed);
...
AbstractFile *
createAbstractFileFromComp (AbstractFile * file)
{
	InfoComp *info;
	...
	uint32_t real_uncompressed =
		decompress_lzss (info->buffer, compressed,
				 info->header.length_compressed);
...
AbstractFile *
duplicateCompFile (AbstractFile * file, AbstractFile * backing)
{
	InfoComp *info;
	AbstractFile *toReturn;
