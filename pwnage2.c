...
int pwnage2()
{
	...
	uint8_t *buf;
	const char header[] =
	    { 0xff, 0xff, 0xff, 0xff, 0xac, 0x05, 0x00, 0x01, 0x55, 0x46, 0x44,
  0x10 };
	uint8_t tmpbuf[255];
	FILE *fp;
	...
	while (i < ((wtf_length + 0x10) + 0x800)) {
		...
		irecv_control_transfer(client, 0x21, 1, c, 0, &buf[i], sl,
				       1000);

		int b = 0;

		while (irecv_control_transfer
		       (client, 0xA1, 3, 0, 0, tmpbuf, 6, 1000) == 6 && b < 5) {
			b++;
			if (tmpbuf[4] == 5)
				break;
		}
