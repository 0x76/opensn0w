	...
	while ((*curChar) != '\0') {
		...
		if (charsInBuffer == 4) {
			...
			if ((decodeLoc + 3) >= decodeBufferSize) {
				decodeBufferSize <<= 1;
				decodeBuffer =
				    (unsigned char *)realloc(decodeBuffer,
							     decodeBufferSize);
			}
...
void
writeBase64(AbstractFile * file, unsigned char *data, size_t dataLength,
		    ...
		    int width)
{
	...
	while (dataLength >= 3) {
		...
		buffer[pos++] =
		    dictionary[(((data[i] << 4) & 0x30) |
				((data[i + 1] >> 4) & 0x0F)) & 0x3F];
		CHECK_LINE_END_STRING();
		buffer[pos++] =
